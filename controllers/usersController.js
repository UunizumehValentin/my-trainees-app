...
const { validateAddUser } = require("../validation/userValidations");
...
const userLogin = async (req, res) => {
  ...
  const verifiedPassword = await bcrypt.compare(req.body.password);
  if (!verifiedPassword) return res.status(404).send("invalid or password");
